 ...
 * @returns {electionModel}
 */
const setCreateElectionVariables = async (req, election) => {
  if (req.tokenInfo) {
    election.createdBy = session.getSessionLoginID(req);
    election.userId = session.getSessionLoginID(req);
    election.userName = session.getSessionLoginName(req);
    election.createdByName = session.getSessionLoginName(req);
    // election.status = "Pending";
    election.userEmail = session.getSessionLoginEmail(req);
    //replace_encryptedFields
    //replace_uniqueIdGeneration
  }

  election.created = Date.now();
  return election;
};

/**
 * set Election update variables
 * @returns {electionModel}
 */
const setUpdateElectionVariables = async (req, election) => {
  if (req.tokenInfo) {
    election.updatedBy = session.getSessionLoginID(req);
    election.updatedByName = session.getSessionLoginName(req);
  }
  //replace_encryptedFields
 ...
 * @returns {Employees}
 */
async function insertElectionData(req, res) {
  req.duplicates = [];
  let obj = req.obj;
  for (let val in obj) {
    try {
      obj[val] = await autoCompleteData(obj[val]);
      let election = new electionModel(obj[val]);
      election = await setCreateElectionVariables(req, election);
      let validateRes = await validateFields(req, election);
      if (validateRes) {
      ...
      obj[val].reason = "Error while creating Election" + err;
      req.duplicates.push(obj[val]);
      delete obj[val];
    }
  }
...
const validateFields = async (req, election) => {
  let isError = false;
...
const validateElectionBulkFields = async (req, res) => {
...
export default {
  setCreateElectionVariables,
  setUpdateElectionVariables,
  insertElectionData,
  validateFields,
  validateElectionBulkFields,
};
