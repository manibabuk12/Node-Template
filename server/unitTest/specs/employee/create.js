...
chai.use(chaiAsPromised);describe("## Check employee creation", () => {
  ...
  it("## Check employee creation", (done) => {
    request(app)
      ...
      .send({
        ...createpostBody,
        phone: generateMatchingString(/^[0-9]{0,10}$/),
      })
...
describe("## TEST VALIDATE FIELDS ", () => {
  ...
  it("CHECK phone IS VALID", (done) => {
    request(app)
      .post("/api/employee")
      .send({ ...createpostBody, phone: generateRandomAlphabets() })
      .set({ Authorization: `Bearer ${authEmployee.getAccessToken()}` })
      .expect(httpStatus.OK)
      .then((res, req = {}) => {
        req.i18nKey = "employeeCreate";
        expect(res.body).to.deep.equal(
          getErroResponseByErrorMessage("Invalid phone!")
        );
        done();
      })
      .catch(done);
