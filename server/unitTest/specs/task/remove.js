...
chai.use(chaiAsPromised);describe("## Check task creation", () => {
  ...
  it("## Check Passing Wrong ParamsId", (done) => {
    request(app)
      .delete(`/api/tasks/${generateRandomNumberString()}?response=true`)
      .set({ Authorization: `Bearer ${authEmployee.getAccessToken()}` })
      .expect(httpStatus.OK)
      .then((res, req = {}) => {
        req.i18nKey = "taskDelete";
  ...
  it("## Should get sucessfully deleted message", (done) => {
    request(app)
      .delete(`/api/tasks/${task.getId()}?response=true`)
      .set({ Authorization: `Bearer ${authEmployee.getAccessToken()}` })
      .expect(httpStatus.OK)
      .then((res, req = {}) => {
        req.i18nKey = "taskDelete";
